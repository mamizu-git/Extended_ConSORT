    ...
    (match e with
     ...
     | EApp (id',es) -> 
       ...
       let g1 ftid_ft e = 
         ...
         | (RawId id_arg, FTRef ((FTInt VarPred),el,eh,f)) -> 
          ...
          [Imply(Ands((ptrpred id_e (FV "i") fvs ifel) :: List.map (fun id -> IntPred(id, id::(lookup id !intpred_env))) ids_depended), PtrVarPred(num, id_arg, "b", (FV "i"), ids_depended))] (* "m" の扱い*)
        | (RawId id_arg, FTInt VarPred) -> 
          let FV id_e = exp_to_smtlib e in
          let fvs_int = lookup id_e !intpred_env in 
          intpred_env := (id_arg, []) :: !intpred_env; 
          let num = lookup id' fun_num in
          [Imply(IntPred(id_e, "v" :: fvs_int), IntVarPred(num, id_arg, ["v"]))] (* "v" の扱い*)
        | (HashId id_arg, FTInt VarPred) -> 
         ...
         | (RawId _, FTRef ((FTInt sl),_,_,_)) -> 
           ...
           [Imply(Ands((ptrpred id_e (FV "i") fvs ifel) :: List.map (fun id -> IntPred(id, id::(lookup id !intpred_env))) ids_depended), n_sl)] (* "m" の扱い*)
         | (RawId _, FTInt sl) -> 
           let FV id_e = exp_to_smtlib e in
           let fvs_int = lookup id_e !intpred_env in 
           [Imply(IntPred(id_e, "v" :: fvs_int), sl)] (* "v" の扱い*)
         | (HashId _, FTInt sl) -> 
           let FV id_e = exp_to_smtlib e in
           let fvs_int = lookup id_e !intpred_env in 
           [Imply(IntPred(id_e, "v" :: fvs_int), sl)] (* "v" の扱い*)
     ...
     | e -> 
       ...
       let rec find_hash fvs' h r = 
         (match fvs' with
          | fv :: rest -> 
            if List.mem fv fvs then find_hash rest (fv::h) r else find_hash rest h (fv::r) (* "n" の扱い　*)
          | [] -> (h, r)) 
       in
  ...
  (* | CHCApp (id_fn,es,l) -> 
    ...
    let g1 ftid_ft e = 
      ...
      | (RawId id_arg, FTRef ((FTInt VarPred),_,_,_)) -> 
       ...
       [Imply(Ands([ptrpred id_e (FV "i") fvs; IntPred("m", "m" :: fvs)]), PtrVarPred(id_arg, 0, (FV "i"), (FV "m")))] (* "m" の扱い*)
     | (RawId id_arg, FTInt VarPred) -> 
       let FV id_e = exp_to_smtlib e in
     ...
     | (HashId id_arg, FTInt VarPred) -> 
      ...
      | (RawId _, FTRef ((FTInt sl),_,_,_)) -> 
        ...
        let n_sl = smtlib_subst subst sl in
        let fvs = lookup "m" !intpred_env in 
        [Imply(Ands([ptrpred id_e (FV "i") fvs; IntPred("m", "m" :: fvs)]), n_sl)] (* "m" の扱い*)
      | (RawId _, FTInt sl) -> 
        let FV id_e = exp_to_smtlib e in
        let fvs = lookup id_e !intpred_env in 
    ...
    let g2 ftid_ft e = 
      ...
      | (RawId id_arg, FTRef ((FTInt VarPred),_,_,_)) -> 
        ...
        let fvs = lookup "m" !intpred_env in 
        new_id id_e l;
        [Imply(Ands([PtrVarPred(id_arg, 0, (FV "i"), (FV "m")); IntPred("m", "m" :: fvs)]), ptrpred id_e (FV "i") fvs)] (**)
      | (RawId _, FTRef ((FTInt sl),_,_,_)) -> 
        let FV id_e = exp_to_smtlib e in
        let n_sl = smtlib_subst subst sl in
        let fvs = lookup "m" !intpred_env in 
        new_id id_e l;
        [Imply(Ands([n_sl; IntPred("m", "m" :: fvs)]), ptrpred id_e (FV "i") fvs)] (**)
      | (RawId _, FTInt _) -> 
        []
      | (HashId _, FTInt _) -> 
        []
